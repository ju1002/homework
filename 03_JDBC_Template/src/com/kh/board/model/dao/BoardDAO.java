...
public class BoardDAO {
	public int insertBoard(Connection conn , Board board) {
		...
		result = pstmt.executeUpdate(); //결과값 받음
	...
	public List<Board> selectBoardList(Connection conn){
		...
		String sql = """
				...
				""";
		try {
			pstmt = conn.prepareStatement(sql); //실행이야
	...
	public Board selectBoard(Connection conn, int boardNo) {
		...
		String sql = """
						SELECT
						 ...
						 FROM
						      BOARD
						  JOIN
						       MEMBER ON (USERNO = BOARD_WRITTER)
						 WHERE
						       DELETE_YN = 'N'
						   AND
						       BOARD_NO = ?
		...
		try {
			pstmt = conn.prepareStatement(sql); //안에 담아놔야지 아니면 null이잖아
